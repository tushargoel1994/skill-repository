"""Price trend of a target supplier vs. all other suppliers, split into one
line chart per component the target supplier serves (all components, since
no --component filter is accepted here — see supplier_price_trend_by_criticality.py
for the version scoped to one component and split by criticality instead).
...
def print_facet_data(label: str, facet_data: dict, supplier_name: str) -> None:
    print(f"\nPrice Trend by Component — {label}")
    print("-" * 80)
    for comp, d in facet_data.items():
        print(f"\nComponent: {comp}  (target POs: {d['target_po_count']}, peer POs: {d['peer_po_count']})")
...
def main(file_path, supplier_name,
         image_output="supplier_price_trend_by_component.png",
         json_output="supplier_price_trend_by_component.json") -> None:
    ...
    facet_data = build_facet_data(target_df, peer_df, "component_category")

    print_facet_data(label, facet_data, supplier_name)

    chart_title = f"Price Trend by Component — {label}"
    saved_image = plot_price_trend_grid(facet_data, "Component", supplier_name, chart_title, image_output)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-output", default="supplier_price_trend_by_component.png")
    parser.add_argument("--json-output", default="supplier_price_trend_by_component.json")
